...
def create_model():
    ...
    def obj_expression(model):
        ...
        return (
                sum(
                    model.pTransportationCosts[iSource, iCustomer] * model.vQuantityExchangedSourceCustomer[iSource, iCustomer]
                    for iSource in model.sSources
                    for iCustomer in model.sCustomers
                    if (iSource, iCustomer) in model.sSources_sCustomers
                )
...
def print_conclusions_sources_sensibility_analysis(reduced_cost, source):
...
def print_conclusions_customers_sensibility_analysis(shadow_price, customer):
    ...
    if shadow_price > 0:
        print("The total transportation cost would be increased in",
              shadow_price,
              "euros for each additional ton demanded in", customer)
...
def solve_problem_and_print_results(data_file):
    ...
    if ((results.solver.termination_condition == TerminationCondition.optimal) and
        ...
        dict_quantity_sources_customers = {(iSource, iCustomer): value(instance.vQuantityExchangedSourceCustomer[iSource, iCustomer])
                                           for iSource in instance.sSources
                                           for iCustomer in instance.sCustomers
                                           if (iSource, iCustomer) in instance.sSources_sCustomers and value(instance.vQuantityExchangedSourceCustomer[iSource, iCustomer]) > 0
                                           }
        ...
        dict_source_sensibility_analysis = {iSource: [value(instance.pSourceProduction[iSource]), value(instance.c01_production_limit_satisfaction[iSource]), value(instance.dual[instance.c01_production_limit_satisfaction[iSource]])]
                                            for iSource in instance.sSources
                                            }
        df_source_sensibility_analysis = pd.DataFrame(dict_source_sensibility_analysis.values(),
                                                      index=pd.Index(dict_source_sensibility_analysis.keys()),
                                                      columns=['Capacity', 'Shipped', 'Reduced Cost']
                                                      ).reset_index(names=['Source'])
        print(df_source_sensibility_analysis)

        df_source_sensibility_analysis.apply(
            lambda row: print_conclusions_sources_sensibility_analysis(row['Reduced Cost'], row['Source']), axis=1)
        print("\n")

        print("Customers sensibility analysis:")
        dict_customers_sensibility_analysis = {iCustomer: [value(instance.pCustomerDemand[iCustomer]), value(instance.c02_demand_limit_satisfaction[iCustomer]), value(instance.dual[instance.c02_demand_limit_satisfaction[iCustomer]])]
                                               for iCustomer in instance.sCustomers
                                               }
        df_customers_sensibility_analysis = pd.DataFrame(dict_customers_sensibility_analysis.values(),
        ...
        df_customers_sensibility_analysis.apply(
            lambda row: print_conclusions_customers_sensibility_analysis(row['Shadow price'], row['Customer']), axis=1)
        print("\n")
...
# Base case
solve_problem_and_print_results("data_0.json")

# Sensibility analysis - sources
# Using base case, we move one ton of supply capacity from Gou to Arn and
# the objetive function improves in 0.2 euros (reduced cost for Arn)
# solve_problem_and_print_results("data_1.json")

# Sensibility analysis - customers - 1
# Using base case, we increase the demand in Lon in one ton and
# we increase one ton of supply capacity in Gou (Gou is the only source for Lon).
# Then the objetive function gets worse in 2.5 euros (shadow price for Lon)
# solve_problem_and_print_results("data_2.json")

# Sensibility analysis - customers - 2
# Using base case, we increase the demand in Ber in one ton and
# we increase one ton of supply capacity in Gou (Arn is the only source for Ber).
# Then the objetive function gets worse in 2.7 euros (shadow price for Ber)
# solve_problem_and_print_results("data_3.json")

# Using the constraint c03_fixed_transportations
# Using base case, we fixed a transportation between Arn and Utr equal to 149 tons.
# The flows between Arn and Ams/Utr and Gou and Ams/Utr change, as well as the objective function.
# As we are doing something different of the base case (where all the flows are free), the solution is worse.
# solve_problem_and_print_results("data_4.json")
